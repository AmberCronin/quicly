...
void ss_search10_interp(quicly_cc_t *cc, const quicly_loss_t *loss, uint32_t bytes, uint64_t largest_acked, uint32_t inflight,
                        uint64_t next_pn, int64_t now, uint32_t max_udp_payload_size)
{
	...
	uint32_t bin_shift = loss->rtt.latest / *bin_time;
	...
	while(now > (*bin_end)) {
		...
		// thus, the number of rounds must be greater than the delv bin count + the bin shift
		// In contrast to the rounded shift (ss-search10.c), this shift is unrounded and thus we
		// may need to add one if a fraction exists (to be able to interpolate a partial previous
		// bin). 
		if((*bin_rounds) > (SEARCH10_DELV_BIN_COUNT + bin_shift + (bin_shift_frac ? 1 : 0))
			...
			for(int i = *bin_rounds; i > (*bin_rounds - SEARCH10_DELV_BIN_COUNT + (bin_shift_frac ? 1 : 0)); i--) {
				sent_sum += sent[((i - bin_shift - (bin_shift_frac ? 1 : 0)) % SEARCH10_SENT_BIN_COUNT)];
			}

			if (bin_shift_frac) {
				// Do interpolation: [pp|llll][SSSSSSS][llll|ee] : S=sent, s=sent_interp, l=lost w/o interp, e=future to bin end
				uint32_t front_interp_time = (now - *bin_end) % (*bin_time);
				uint32_t back_interp_time = (*bin_time - front_interp_time);
				// (sent[i] - sent[i-1])/bin_time
				uint32_t mf = (sent[(*bin_rounds - bin_shift) % SEARCH10_SENT_BIN_COUNT] * front_interp_time) / *bin_time;
				uint32_t mb = (sent[(*bin_rounds - SEARCH10_DELV_BIN_COUNT - bin_shift) % SEARCH10_SENT_BIN_COUNT] * back_interp_time) / *bin_time;
				sent_sum += (mf);
				sent_sum += (mb);
			}
			if (sent_sum == 0) {
...
quicly_ss_type_t quicly_ss_type_search10_interp = { "search10interp", ss_search10_interp };
